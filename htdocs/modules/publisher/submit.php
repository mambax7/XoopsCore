...
if (0 !== $itemid) {
    ...
    if (!PublisherUtils::IsUserAdmin() || !PublisherUtils::IsUserModerator($itemObj)) {
        if (isset($_GET['op']) && 'del' === $_GET['op'] && !$publisher->getConfig('perm_delete')) {
            $xoops->redirect('index.php', 1, XoopsLocale::E_NO_ACCESS_PERMISSION);
    ...
    if (!(PublisherUtils::IsUserAdmin() || (1 === $publisher->getConfig('perm_submit') && ($xoops->isUser() || (1 === $publisher->getConfig('perm_anon_submit')))))) {
        $xoops->redirect('index.php', 1, XoopsLocale::E_NO_ACCESS_PERMISSION);
    }
...
switch ($op) {
    ...
    case 'post':
        ...
        if (!$itemid) {
            if (_PUBLISHER_STATUS_PUBLISHED === $itemObj->getVar('status') /*$publisher->getConfig('perm_autoapprove'] ==  1*/) {

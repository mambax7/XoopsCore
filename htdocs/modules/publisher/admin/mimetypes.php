...
$aSortBy = ['mime_id' => _AM_PUBLISHER_MIME_ID,
...
function add()
{
    ...
    if (!isset($_POST['add_mime'])) {
        ...
        if (false === !$mime_errors) {
            PublisherUtils::renderErrors($mime_errors, PublisherUtils::makeURI(PUBLISHER_ADMIN_URL.'mimetypes.php', ['op' => 'clearAddSession']));
        }
        ...
        <td class='even'>";
        echo "<input type='radio' name='mime_admin' value='1' ".(1 === $mime_admin ? "checked='checked'" : '').' />'.XoopsLocale::YES;
        echo "<input type='radio' name='mime_admin' value='0' ".(0 === $mime_admin ? "checked='checked'" : '').' />'.XoopsLocale::NO.'
        </td>
        </tr>';
        echo "<tr valign='top'>
        <td class='head'>"._AM_PUBLISHER_MIME_USERF."</td>
        <td class='even'>";
        echo "<input type='radio' name='mime_user' value='1'".(1 === $mime_user ? "checked='checked'" : '').' />'.XoopsLocale::YES;
        echo "<input type='radio' name='mime_user' value='0'".(0 === $mime_user ? "checked='checked'" : '').'/>'.XoopsLocale::NO.'
        </td>
        </tr>';
        echo "<tr valign='top'>
        <td class='head'></td>
...
function edit()
{
    ...
    if (!isset($_POST['edit_mime'])) {
        ...
        //Display any form errors
        if (false === !$mime_errors) {
            PublisherUtils::renderErrors($mime_errors, PublisherUtils::makeURI(PUBLISHER_ADMIN_URL.'/mimetypes.php', ['op' => 'clearEditSession', 'id' => $mime_id]));
        }
        ...
        <td class='head'>"._AM_PUBLISHER_MIME_ADMINF."</td>
        <td class='even'>
        <input type='radio' name='mime_admin' value='1' ".(1 === $mime_admin ? "checked='checked'" : '').' />'.XoopsLocale::YES."
        <input type='radio' name='mime_admin' value='0' ".(0 === $mime_admin ? "checked='checked'" : '').' />'.XoopsLocale::NO.'
        </td>
        </tr>';
        echo "<tr valign='top'>
        <td class='head'>"._AM_PUBLISHER_MIME_USERF."</td>
        <td class='even'>
        <input type='radio' name='mime_user' value='1' ".(1 === $mime_user ? "checked='checked'" : '').' />'.XoopsLocale::YES."
        <input type='radio' name='mime_user' value='0' ".(0 === $mime_user ? "checked='checked'" : '').' />'.XoopsLocale::NO.'
        </td>
        </tr>';
        echo "<tr valign='top'>
        <td class='head'></td>
        ...
        //Validate Mimetype entry
        if (0 === strlen(trim($_POST['mime_ext']))) {
            $has_errors = true;
            $error['mime_ext'][] = _AM_PUBLISHER_VALID_ERR_MIME_EXT;
        }

        if (0 === strlen(trim($_POST['mime_name']))) {
            $has_errors = true;
            $error['mime_name'][] = _AM_PUBLISHER_VALID_ERR_MIME_NAME;
        }

        if (0 === strlen(trim($_POST['mime_types']))) {
            $has_errors = true;
            $error['mime_types'][] = _AM_PUBLISHER_VALID_ERR_MIME_TYPES;
        }
        ...
        if ($has_errors) {
            ...
            header('Location: '.PublisherUtils::makeURI(PUBLISHER_ADMIN_URL.'/mimetypes.php', ['op' => 'edit', 'id' => $mime_id], false));
        }
...
function clearEditSession()
{
    ...
    header('Location: '.PublisherUtils::makeURI(PUBLISHER_ADMIN_URL.'/mimetypes.php', ['op' => 'edit', 'id' => $mimeid], false));
}

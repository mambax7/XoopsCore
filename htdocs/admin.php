...
if ($xoops->getConfig('admin_warnings_enable')) {
    ...
    if (false !== strpos($xoops_path, $xoops_root_path) || false !== strpos($xoops_path, $_SERVER['DOCUMENT_ROOT'])) {
        $error_msg[] = sprintf(XoopsLocale::EF_FOLDER_IS_INSIDE_DOCUMENT_ROOT, $xoops_path);
    }

    $var_path = $xoops->path('var');
    if (false !== strpos($var_path, $xoops_root_path) || false !== strpos($var_path, $_SERVER['DOCUMENT_ROOT'])) {

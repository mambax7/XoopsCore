...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
  ...
  #
  {
    ...
    if (count($this->captionBoxPositionArray) === 4) {
      ...
      if ($this->debug) { throw new Exception('No caption box found.'); } return false;
    }
...
## --------------------------------------------------------
  ...
  #       third party class to save as bmp.
    ...
    if (! is_resource($this->imageResized)) { if ($this->debug) { throw new Exception('saveImage: This is not a resource.'); } throw new Exception(); }
    $fileInfoArray = pathinfo($savePath);
    clearstatcache();
    if (! is_writable($fileInfoArray['dirname'])) {  if ($this->debug) { throw new Exception('The path is not writable. Please check your permissions.'); } throw new Exception(); }
  ...
  private function getTextFont($font)
  {
    ...
    if ($font === null || ! file_exists($font)) {
      ...
      if (! file_exists($font)) {
        ...
        // *** If not, return false
        if ($this->debug) { throw new Exception('Font not found'); } return false;
      }
    }

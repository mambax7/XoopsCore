...
elseif(strpos($x, '*') === false){$e = array_flip(explode(',', $x)); }
 ...
 if(isset($x[1])){
  ...
  foreach($m as $v){
   ...
   if($v[0] === '-' && isset($e[($v = substr($v, 1))]) && ! in_array('+' . $v, $m, true)){unset($e[$v]); }
  }
 }
}
$C['elements'] = & $e;
...
if(! isset($C['schemes']['*'])){
 ...
 if(empty($C['safe'])){$C['schemes']['*'] += ['data' => 1, 'javascript' => 1]; }
}
...
if($GLOBALS['C']['direct_list_nest']){$cS['ol'] = $cS['ul'] = $cS['menu'] += ['menu' => 1, 'ol' => 1, 'ul' => 1]; }
...
for($i = -1, $ci = count($t); ++$i < $ci;){
 ...
 // close tag
 if($s){
  if(isset($cE[$e]) or ! in_array($e, $q, true)){continue; } // Empty/unopen
